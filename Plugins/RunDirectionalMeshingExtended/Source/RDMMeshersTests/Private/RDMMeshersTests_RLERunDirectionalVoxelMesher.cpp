...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRDMMeshersTests_RLERunDirectionalVoxelMesher_CheckValidVoxelSequenceAfterMeshing,
	"RDM.RDMMeshersTests.RLERunDirectionalVoxelMesher.CheckValidVoxelSequenceAfterMeshing",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter
);

bool FRDMMeshersTests_RLERunDirectionalVoxelMesher_CheckValidVoxelSequenceAfterMeshing::RunTest(const FString& Parameters)
{
	// Prepare
	...
	for (int i = 0; i < VoxelCount - 1; i++)
	{
		...
		bool EqualVoxel = NextVoxel.Voxel != Voxel.Voxel;
		TestTrue("Each voxel needs to be unique after meshing.", EqualVoxel);
	}
